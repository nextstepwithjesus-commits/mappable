...
const PASSAGES: { ref: string; vol: string[] }[] = [
  ...
  { ref: 'Чис 13:5-16', vol: ['18'] },
  { ref: 'Чис 25:14-15', vol: ['11'] },
  { ref: 'Чис 26:5-62', vol: ['06', '08', '09', '10'] },
  { ref: 'Суд 8:29-32', vol: ['11'] },
  { ref: 'Суд 10:1-5', vol: ['11'] },
  { ref: 'Суд 12:8-15', vol: ['11'] },
...
const STOP = new Set(
  [
    ...
    'Всего', 'Всех', 'Подле', 'Далее', 'Божием', 'Главы', 'Первые', 'Нефинеи', 'Нефинеев', 'За', 'Трое', 'Тогда', 'Над', 'Так', 'Да', 'Пришли', 'Вооруженные', 'Они-то', 'Божие', 'Филистимские', 'Господню', 'Господней', 'Семейство', 'Семейства', 'Начальник', 'Князь', 'Князья', 'Царь', 'Цари', 'Жена', 'Жены', 'Дочь', 'Дочери', 'Первенец', 'Братья', 'Потом', 'Там', 'Таковы',
  ].map((w) => norm(w)),
);

const ORDINAL = /^(первый|второй|третий|четвертый|пятый|шестой|седьмой|восьмой|девятый|десятый|одиннадцатый|двенадцатый|главный)$/;
...
// запись «=Слово» — только точная форма (место «Азмавефа» не должно скрывать лицо Азмавефа); остальные — с окончаниями
const ignoreExact = new Set(ignore.filter((w) => w.startsWith('=')).map((w) => norm(w.slice(1))));
const ignoreRes = ignore.filter((w) => !w.startsWith('=')).map((w) => nameMatcher(w));
...
for (const ps of PASSAGES) {
  ...
  for (const v of pr.verses) {
    ...
    while ((m = re.exec(clean))) {
      ...
      // прозвища по месту и роду («Гараритянин», «Пелонитянин») и порядковые числительные — не лица
      if (/(янин|янина|яне|янка|итянин|итяне)$/.test(norm(w)) || ORDINAL.test(norm(w))) continue;
  ...
  for (const { w, v } of words) {
    if (ignoreExact.has(norm(w)) || ignoreRes.some((r) => r.test(' ' + norm(w) + ' '))) continue;
    counted++;
